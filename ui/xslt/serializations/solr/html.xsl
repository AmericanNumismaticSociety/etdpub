...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="start"
		select="if (number(doc('input:request')/request/parameters/parameter[name='start']/value)) then doc('input:request')/request/parameters/parameter[name='start']/value else
		0"
		as="xs:integer"/>
	<xsl:param name="lang"
	...
	<xsl:variable name="id_space"
		select="if (/content/config/ark/@enabled='true') then concat('ark:/', /content/config/ark/naan) else 'id'"/>
	...
	<xsl:template match="/">
		<html lang="en">
			<head>
				<title>
					<xsl:value-of select="/content/config/title"/>: Browse </title>
				...
				<link rel="stylesheet" href="{$display_path}ui/css/bootstrap-multiselect.css"
					type="text/css"/>
			...
			<div class="row">
				...
				<div class="col-md-9">
					<xsl:choose>
						...
						<xsl:otherwise>
							<p>No results found for this query. <a href="{$display_path}">Clear
									search</a>.</p>
						</xsl:otherwise>
					</xsl:choose>
				</div>
	...
	<xsl:template match="doc">
		...
		<div class="result-doc row">
			...
			<xsl:choose>
				<xsl:when test="//lst[@name='highlighting']/lst[@name=$id]/arr[@name='text']/str">
					...
					<div class="col-md-6">
						<xsl:variable name="snippets"
							select="saxon:parse(concat('&lt;div&gt;', replace(string-join(//lst[@name='highlighting']/lst[@name=$id]/arr[@name='text']/str, '...'), '&amp;',
							'&amp;amp;'), '&lt;/div&gt;'))"/>
				...
				<xsl:otherwise>
					<div class="col-md-{if (str[@name='thumbnail_image']) then '10' else '12'}">
						<xsl:call-template name="metadata"/>
					</div>
					<xsl:if test="str[@name='thumbnail_image']">
						<div class="col-md-2">
							<a href="{concat($url, $id_space, '/', $id)}">
								<img src="{$display_path}{str[@name='thumbnail_image']}" alt="thumbnail" style="max-width:100%"/>
							</a>
						</div>
					</xsl:if>
				</xsl:otherwise>
	...
	<xsl:template match="lst[@name='facet_fields']">
		<!-- separate document metadata from subjects -->
		...
		<xsl:apply-templates select="lst[descendant::int][@name = $facets//facet[@type='doc']]"
			mode="facet"/>

		<h4>Subjects</h4>
		<xsl:apply-templates select="lst[descendant::int][@name = $facets//facet[@type='subject']]"
			mode="facet"/>
	...
	<xsl:template match="lst" mode="facet">
		...
		<select id="{@name}-select" title="{$title}" q="{$q}"
			...
			<xsl:if test="contains($q, @name)">
				<xsl:copy-of
					select="document(concat($url, 'get_facets/?q=', encode-for-uri($q), '&amp;category=', @name, '&amp;sort=index&amp;limit=-1'))//option"
				/>
			</xsl:if>
		</select>
	</xsl:template>
	...
	<xsl:template name="metadata">
		<dl class="dl-horizontal">
			...
			<xsl:if test="arr[@name='series_facet']">
				<dt>Series</dt>
				<dd>
					<xsl:value-of select="string-join(arr[@name='series_facet']/str, ',')"/>
				</dd>
			</xsl:if>
			...
			<input type="text" id="search_text" class="form-control" placeholder="Keyword Search">
				<xsl:if test="count($tokenized_q[not(contains(., ':'))]) &gt; 0">
					<xsl:attribute name="value"
						select="string-join($tokenized_q[not(contains(., ':'))], ' ')"/>
				</xsl:if>
			</input>
	...
	<xsl:template name="remove_facets">
		<div class="col-md-12 filter-container">
			...
			<xsl:if test="count($tokenized_q) &gt; 2">
				<div class="stacked_term row">
					<div class="col-md-12">
						<a id="clear_all" href="{$display_path}results"><span
								class="glyphicon glyphicon-remove"/>Clear All Terms</a>
					</div>
				</div>
			</xsl:if>
		</div>
						...
						<xsl:choose>
							<xsl:when test="$numFound - $start &gt; $rows">
								<a class="btn btn-default" title="Next"
									href="results?q={encode-for-uri($q)}&amp;start={$next}{if (string($sort)) then concat('&amp;sort=', $sort) else ''}">
									<span class="glyphicon glyphicon-forward"/>
								</a>
								<a class="btn btn-default"
									href="results?q={encode-for-uri($q)}&amp;start={($total * $rows) - $rows}{if (string($sort)) then concat('&amp;sort=', $sort) else ''}">
