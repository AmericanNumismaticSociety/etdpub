...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema"
		...
		<div class="result-doc row">
			...
			<xsl:choose>
				<xsl:when test="//lst[@name='highlighting']/lst[@name=$id]/arr[@name='text']/str">
					...
					<div class="col-md-6">
						<xsl:variable name="snippets" select="saxon:parse(concat('&lt;div&gt;', replace(string-join(//lst[@name='highlighting']/lst[@name=$id]/arr[@name='text']/str, '...'), '&amp;',
							'&amp;amp;'), '&lt;/div&gt;'))"/>
	...
	<xsl:template match="lst[@name='facet_fields']">
		...
		<xsl:apply-templates select="lst[descendant::int][@name = $facets//facet[@type='doc']]" mode="facet"/>
		
		<h4>Subjects</h4>
		<xsl:apply-templates select="lst[descendant::int][@name = $facets//facet[@type='subject']]" mode="facet"/>
