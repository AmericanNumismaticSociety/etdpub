...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1" xml:lang="en">
			...
			<docTitle>
				<text><xsl:value-of select="/tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title"/></text>
			</docTitle>
			<navMap>				
				<xsl:for-each select="descendant::tei:body/tei:div1">
					...
					<navPoint id="{$marker}"
						...
						<xsl:for-each select="tei:div2">
							<navPoint id="{$marker}-{format-number(position(), '000')}" playOrder="{count(preceding-sibling::tei:div2) + count(../preceding-sibling::tei:div1) + count(../preceding-sibling::tei:div1/tei:div2) + 2}">
								<navLabel>
									<text>
										<xsl:choose>
											<xsl:when test="tei:head">
