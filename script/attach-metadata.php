...
//load XML
$crossrefConfig = '/usr/local/projects/etdpub/crossref-config.xml';
...
if (file_exists($crossrefConfig)) {
	...
	$curl = "curl -H 'User-Agent: AmericanNumismaticSociety/XForms' -F operation=doMDUpload -F login_id={$username} -F login_passwd={$password} -F fname=@/tmp/{$file} {$url}";
